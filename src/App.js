...
class App extends Component {
  ...
  getActiveTabIdList() {
    ...
    const filteredList = imagesList.filter(
      each => each.category === isActivetabId,
    )
  ...
  onTimerDecrement = () => {
    ...
    if (timer === 0) {
      this.clearTimer()
      timerCall = true
      this.setState({isGameCompleted: true, timer})
    } else {
  ...
  timerInterval = () => {
    this.intervalId = setInterval(this.onTimerDecrement, 1000)
    timerCall = false
  }
  ...
  render() {
    ...
    const filteredList = this.getActiveTabIdList()
          ...
          {isGameCompleted ? (
            <div className="score-card">
              ...
              >
                ...
                />
                <p className="text-play-again">PLAY AGAIN</p>
              </button>
            </div>
          ) : (
            <>
